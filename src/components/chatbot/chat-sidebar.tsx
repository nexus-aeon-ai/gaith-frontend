...
import type { Chat, Conversation } from "@/lib/types";
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <h3 className="font-medium text-xs sm:text-sm truncate">
                      Client: {chat.title.slice(0, 20)}{chat.title.length > 20 ? "..." : ""}
                    </h3>
                    <span className="text-xs text-muted-foreground flex-shrink-0 ml-1 sm:ml-2">
                      {chat.timestamp}
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground line-clamp-2">{chat.lastMessageText}</p>
                </div>
              </div>
            </Button>
          ))}
