...
//           </Label>
//           <Select onValueChange={(value) => setValue("status", value as "Not Started" | "In Progress" | "Completed")}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-2 sm:space-y-3">
        ...
        <div>
          ...
          <Input
            type="date"
            id="due-date"
            className={cn("bg-input mt-1 sm:mt-2 text-xs sm:text-sm h-8 sm:h-10", errors.dueDate ? "border-red-500" : "")}
            {...register("dueDate")}
          />
          {errors.dueDate && <p className="text-red-500 text-xs mt-1">{errors.dueDate.message}</p>}
        </div>
